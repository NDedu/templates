...
func (model *BookModel) Search(ctx context.Context, query string) ([]*SearchResult, error) {
	...
	sql := `
		SELECT uuid, name,
			CASE WHEN LOWER(name) LIKE LOWER($1) || '%' ESCAPE '\' THEN 0
			     WHEN LOWER(name) LIKE '%' || LOWER($1) || '%' ESCAPE '\' THEN 1
			     ELSE 2
			END AS rank
		FROM books
	...
	rows, err := model.db.QueryContext(ctx, sql, escaped)
	if err != nil {
...
func (model *BookModel) Update(ctx context.Context, book *Book) error {
	...
	query := `update books set name = $1, description = $2, updated_at = $3 where id = $4`
