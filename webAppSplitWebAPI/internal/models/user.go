...
func (model *UserModel) Get(ctx context.Context, id int) (*User, error) {
	...
	var user User
	query := "select id, uuid, first_name, last_name, email, role, is_active, created_at, updated_at from users where id = $1"

	row := model.db.QueryRowContext(ctx, query, id)
	err := row.Scan(&user.ID, &user.UUID, &user.FirstName, &user.LastName, &user.Email, &user.Role, &user.IsActive, &user.CreatedAt, &user.UpdatedAt)
	if err != nil {
...
func (model *UserModel) GetByUUID(ctx context.Context, uuid string) (*User, error) {
	...
	var user User
	query := "select id, uuid, first_name, last_name, email, role, is_active, created_at, updated_at from users where uuid = $1"
...
func (model *UserModel) GetUserByEmail(ctx context.Context, email string) (*User, error) {
	...
	var user User
	query := "select id, uuid, first_name, last_name, email, password, role, is_active, created_at, updated_at from users where email = $1"

	row := model.db.QueryRowContext(ctx, query, email)
	err := row.Scan(&user.ID, &user.UUID, &user.FirstName, &user.LastName, &user.Email, &user.Password, &user.Role, &user.IsActive, &user.CreatedAt, &user.UpdatedAt)
	if err != nil {
...
func (model *UserModel) GetAll(ctx context.Context) ([]*User, error) {
	...
	query := `
		select
			id, uuid, first_name, last_name, email, role, is_active, created_at, updated_at
		from
			users
		order by
...
func (model *UserModel) Update(ctx context.Context, user *User) error {
	...
	query := `update users set first_name = $1, last_name = $2, email = $3, role = $4, is_active = $5, updated_at = $6 where id = $7`
