...
func OpenDB(dsn string) (*sql.DB, error) {
	...
	// FIX: sqlite serialises writes — with 25 open conns concurrent writes will hit "database is locked"
	// add WAL + a busy timeout to the dsn (?_journal_mode=WAL&_busy_timeout=5000) or move to postgres for production
