...
func (model *TokenModel) Insert(ctx context.Context, t *tokens.Token, u *User) error {
	...
	_, err := model.db.ExecContext(ctx, `DELETE FROM tokens WHERE user_id = ? AND scope = ? AND expiry < ?`, u.ID, t.Scope, time.Now())
	if err != nil {
...
func (model *TokenModel) DeleteByToken(ctx context.Context, plainToken string) error {
	...
	hash := sha256.Sum256([]byte(plainToken))
	_, err := model.db.ExecContext(ctx, `DELETE FROM tokens WHERE token_hash = ?`, hash[:])
	return err
}
